...
client.on('message', async message => {
  ...
  if(!toMute) return message.channel.send("Please specify an user !");
    ...
    if(toMute.roles.cache.has(role.id)) return message.channel.send('This user is already muted');
  ...
  if(!toMute.roles.cache.has(role.id)) return message.channel.send('This user is not muted');

  if(toMute.roles.cache.has(role.id)) {
      await(toMute.roles.remove(role));
      message.channel.send('This user has been unmuted');
  }
         ...
         let messagecount = parseInt(args[0]);
           ...
           message.channel.messages.fetch({
               limit: Math.min(messagecount + 1, 100, 200)
                  ...
                  message.channel.bulkDelete(messages.length);
                  message.channel.send("`" + messages + "` messages deleted!");
             })
