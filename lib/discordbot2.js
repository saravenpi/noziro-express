...
client.on('message', async message => {
          ...
          //et la le say ( il pue la merde pcq c un vieux )
           ...
           else if(cmd.startsWith(prf + 'say')){
           if(!own) return;
           if (typech == "text"){
             if(message.member.hasPermission('MANAGE_WEBHOOKS')){
               ...
               if(!argss) return message.delete().then(csl.log('[','AIE'.red,']','hola l ami, (une mention) + un argument est nécessaire'));
                   message.delete();
                     message.channel.createWebhook(message.author.username,message.author.displayAvatarURL)
                                 .then(webhook => {
                                    ...
                                    }).catch(error => console.log('[','WARN'.yellow,']','une erreur est survenue n affectant pas le système'));
                                 })
               } else {
               const argss = args.slice(1).join(" ");
                 if(!argss) return message.delete().then(csl.log('[','AIE'.red,']','hola l ami, (une mention) + un argument est nécessaire'));
                   ...
                   message.delete().then(csl.log('[','CALMOS'.red,']','tu n as pas la permission!'))
                 }
           } else {
                 message.delete().then(console.log('[','HEHO'.red,']','cette commande est valable uniquement sur un server'))
               }
