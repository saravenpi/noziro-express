...
client.on('message', async message => {
          ...
          antiSpam.message(message)
              ...
              if (message.content.startsWith("!ban")) {
                          ...
                          var banConfirmationEmbed = new Discord.RichEmbed()
                          .setColor('RED')
                ...
                if (message.content.startsWith("!kick")) {
                            let args = message.content.split(" ").slice(1);
                            let user = message.mentions.members.first(); // returns the user object if an user mention exists
                            let kickReason = args.slice(1).join(' '); // Reason of the kick (Everything behind the mention)
                            ​
                            // Check if an user mention exists in this message
                            if (!user) {
                            try {
                            ...
                            if (!reason) return message.reply('You forgot to enter a reason for this kick!'); // Check if a reason has been given by the message author
                            //if (!message.guild.member(user).bannable) return message.reply('You can\'t kick this user because you the bot has not sufficient permissions!'); // Check if the user is kickable with the bot's permissions
                            try {
                            ...
                            var kickConfirmationEmbed = new Discord.RichEmbed()
                            .setColor('RED')
                            .setDescription(`✅ ${user.tag} has been successfully kicked!`)
                            .addField("Reason: ", reason)
                            message.channel.send(kickConfirmationEmbed); // Sends a confirmation embed that the user has been successfully kickeds
